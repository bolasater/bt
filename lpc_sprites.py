...
class Mob(arcade.Sprite):
        ...
        for a in cls.action:
                for b in cls.body_part:
                    ...
                    for p in parts:
                        ...
                        if q.exists():
                            fn = q.as_posix()
                            cls.textures.setdefault(a,{}).setdefault(b,{}).setdefault(p,arcade.load_textures(fn,sublocns))
    ...
    def __init__(self):
        ...
        # load textures
        if Mob.textures == {}:
            Mob.load_mob_textures(Mob)
        self.textures={}
        self.textures.setdefault('walking', Mob.textures['walkcycle']['BODY'][self.body])
    ...
    def advance_frame(self):
        self.frame = (self.frame + 1) % Mob.frames_per_sheet

    def update_texture(self):
        self.cur_texture =  self.textures[self.action][self.facing*Mob.frames_per_sheet + self.frame]
...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        # self.mob = Mob()
        # self.mob.center_x = (
        #     self.tile_map.tile_width * TILE_SCALING * PLAYER_START_X + 50
        # )
        # self.mob.center_y = (
        #     self.tile_map.tile_height * TILE_SCALING * PLAYER_START_Y
        # )
        # self.scene.add_sprite(LAYER_NAME_PLAYER, self.mob)
